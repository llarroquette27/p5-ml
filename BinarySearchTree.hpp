...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int height_impl(const Node *node) {
    ...
    // Left side height
    int left_height = 1 + height_impl(node->left);

    // Right side height
    int right_height = 1 + height_impl(node->right);
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    // Base cases
    if (!node->left && less(item, node->datum)) {
      Node *n = new Node;
      n->datum = item;
      n->left = nullptr;
      n->right = nullptr;
    ...
    else if (!node->right && less(node->datum, item)) {
      Node *n = new Node;
      n->datum = item;
      n->left = nullptr;
      n->right = nullptr;
  ...
  static Node * min_greater_than_impl
  (Node *node, const T &val, Compare less) {
    // Base cases
    if (!node) {
      return node;
    }
    if (less(val, node->datum) && !node->left) {
      return node;
    }
    ...
    && !less(val, node->datum))) {
      Node *n = min_greater_than_impl(node->right, val, less);
      return n;
    }
    else {
      Node *n = min_greater_than_impl(node->left, val, less);
      if (n && less(n->datum, node->datum)) {
        return n;
      }
      else {
        return node;
